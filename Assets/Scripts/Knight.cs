...
public class Knight : MonoBehaviour
{
    ...
    void Update()
    {
        // not a physics func - upadate as many times as the sys is capable
    ...
    private void FixedUpdate()
    {
        ...
        if (!damageable.LockVelocity)
        {
            if (canMove && touchingDirections.IsGrounded)
            {
                ...
                rb.velocity = new Vector2(Mathf.Clamp(rb.velocity.x + (walkAcceleration * WalkableDirectionVector.x * Time.fixedDeltaTime), -maxSpeed, maxSpeed), rb.velocity.y);
            }
            else
            {
